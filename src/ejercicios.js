...
function obtenerCanal(matriz, canal) {
  ...
  // copiarMatriz() es una función que ya existe en tu repo
  const resultado = copiarMatriz(matriz);
  
  // 3. Para cada pixel, usar solo el valor del canal seleccionado
  for (let i = 0; i < resultado.length; i++) {
    for (let j = 0; j < resultado[i].length; j++) {
      const valor = matriz[i][j][canal]; // extrae r, g o b
  ...
  return resultado; // MATRIZ EN ESCALA DE GRISES CON SOLO ESE CANAL
}
...
function ajustarBrillo(matriz, factor) {
  ...
  return resultado; // MATRIZ CON BRILLO AJUSTADO
}
...
function voltearHorizontal(matriz) {
  ...
  for (let i = 0; i < resultado.length; i++) {
    resultado[i].reverse();  // efecto espejo
  }

  return resultado; // MATRIZ ESPEJO
}
...
function rotar90Grados(matriz) {
  ...
  // 1. Crear matriz transpuesta manualmente (porque son objetos)
  const transpuesta = [];
  for (let j = 0; j < ancho; j++) {
    const fila = [];
    for (let i = 0; i < alto; i++) {
      fila.push(matriz[i][j]); // pixel[i][j] → pixel[j][i]
    }
    transpuesta.push(fila);
  }

  // 2. Voltear horizontalmente cada fila de la transpuesta
  for (let i = 0; i < transpuesta.length; i++) {
    transpuesta[i].reverse();
  }

  return transpuesta; // MATRIZ ROTADA 90° HORARIO
}
...
function aplicarSepia(matriz) {
  ...
  // 2. Recorrer cada pixel y aplicar transformación
  for (let i = 0; i < resultado.length; i++) {
    for (let j = 0; j < resultado[i].length; j++) {
      const p = matriz[i][j];
      ...
      // Limitar valores 0–255 para evitar overflow
      resultado[i][j] = {
        r: limitarValorColor(r),
        g: limitarValorColor(g),
        b: limitarValorColor(b),
        a: p.a  // alpha no se cambia
      };
    }
  }

  return resultado; // Imagen sepia
}
